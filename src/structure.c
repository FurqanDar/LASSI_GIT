...
/// (i,j,k) position
int Lat_Ind_FromCoords(const int i, const int j,
                       const int k) { // Lattice index from 3D to 1D array
...
int Lat_Ind_FromVec(const int *xArr) { // Just the vector form of the above
  ...
  return xArr[POS_X] +
         nBoxSize[POS_X] * (xArr[POS_Y] + nBoxSize[POS_Y] * xArr[POS_Z]);
}
...
int Lat_Ind_OfBead(const int beadID) {
  return bead_info[beadID][POS_X] +
         nBoxSize[POS_X] * (bead_info[beadID][POS_Y] +
                            nBoxSize[POS_Y] * bead_info[beadID][POS_Z]);
}

// Note that the distance functions account for periodic boundaries

/// Dist_PointTotPoint_Float - euclidean distance between the vectors (arrays)
/// f1 and f2 where f1 and f2 are floats. \param f1 \param f2 \return
...
/// Dist_PointToPoint - euclidean distance between the vectors (arrays) f1 and
/// f2 where f1 and f2 are integer points. \param f1 \param f2 \return
float Dist_PointToPoint(const int *f1, const int *f2) {
  ...
  return sqrtf(
      (float)(d[POS_X] * d[POS_X] + d[POS_Y] * d[POS_Y] + d[POS_Z] * d[POS_Z]));
}
...
/// Dist_BeadToPoint_Double - euclidean distance between beadID and the double
/// array f1. \param beadID \param f1 \return
...
/// Dist_BeadToPoint_Float - euclidean distance between beadID and the float
/// array f1. \param beadID \param f1 \return
float Dist_BeadToPoint_Float(const int beadID, const float *f1) {
  ...
  return sqrtf(
      (d[POS_X] * d[POS_X] + d[POS_Y] * d[POS_Y] + d[POS_Z] * d[POS_Z]));
}
  ...
  for (i = 0; i < tot_beads; i++) {
    ...
    if (naTotLattice[Lat_Ind_FromVec(tmpR)] == -1) {
      printf("Lattice Position for bead %d is empty! Chain: %d\n", i,
             bead_info[i][BEAD_CHAINID]);
      return i + 1;
    }
    if (i - naTotLattice[Lat_Ind_FromVec(tmpR)] !=
        0) { // This means there is a mismatch between where the bead is and
             // where the lattice thinks the bead is
      printf("Bead position and lattice value not the same. Crashing\t\t");
      printf("B1:%d B2:%d\t C1:%d C2:%d\n", i,
             naTotLattice[Lat_Ind_FromVec(tmpR)], bead_info[i][BEAD_CHAINID],
             bead_info[naTotLattice[Lat_Ind_FromVec(tmpR)]][BEAD_CHAINID]);
      return i + 1;
    }
    while (topo_info[i][idx] != -1 && idx < MAX_BONDS) {
      bondPart = topo_info[i][idx];
      if (Dist_BeadToBead(i, bondPart) > 1.74 * linker_len[i][idx]) {
        printf("Bad beads! %d\t(%d %d %d)\t\tTopo:(%d %d "
               "%d)\t\tLinkers:(%.5f\t%.5f\t%.5f)\n",
               i, bead_info[i][0], bead_info[i][1], bead_info[i][2],
               topo_info[i][0], topo_info[i][1], topo_info[i][2],
               (float)linker_len[i][0], (float)linker_len[i][1],
               (float)linker_len[i][2]);
        printf("\t\t\t\t\t-------------------->\t\t%f\tSHOULD BE\t%f\n",
               Dist_BeadToBead(i, bondPart), 1.74 * (float)linker_len[i][idx]);
        printf("Bad beads! %d\t(%d %d %d)\t\tTopo:(%d %d "
               "%d)\t\tLinkers:(%.5f\t%.5f\t%.5f)\n\n",
               bondPart, bead_info[bondPart][0], bead_info[bondPart][1],
               bead_info[bondPart][2], topo_info[bondPart][0],
               topo_info[bondPart][1], topo_info[bondPart][2],
    ...
    if (bead_info[i][BEAD_FACE] != -1) {
      ...
      if (i != bead_info[bead_info[i][BEAD_FACE]][BEAD_FACE]) {
        printf("Bad bond!\n\t%d %d %d %f\nCrashing.\n", i,
               ...
               fEnergy[bead_info[i][BEAD_TYPE]]
                      [bead_info[bead_info[i][BEAD_FACE]][BEAD_TYPE]][E_SC_SC]);
        return i + 1;
      }
...
/// GyrTensor_ClusterSpecific - calculates the total Gyration Tensor for a given
/// cluster \param ClusSize - the total size of the cluster. \param ClusIndex -
/// the index on naCluster where the cluster is stored. THIS IS VERY OLD AND
...
void GyrTensor_ClusterSpecific(int ClusSize, int ClusIndex) {
  ...
  // ClusSize is the size of the cluster -- obviously -- whereas ClusIndex tell
  // us where in naCluster the chain indecies are located.
  // naCluster[ClusIndex][0-ClusSize] is all the chainID's I need for the
  // calculation Remember that the Gyration Tensor is a 3x3 symmetric object so
  // we only need 6 numbers.
  int i, k, j, j2; // Basic indecies for loops
  for (i = 0; i < 7; i++) {
    fGyrTensor[i] = 0.;
  }                  // Initializing
  int firstB, lastB; // Tracks the first and last bead of the given chain
  float tot_COM[POS_MAX] = {
      0.};        // This is where we shall store the COM of the cluster.
  int NumRes = 0; // Tracks how many residues are in this cluster

  // The only thing one needs to be careful about is to take PBC into account;
  // the rest is tedium. Use good old differential geometry to map each
  // coordinate to two new coordinates, and unpack at the end.
  ...
  for (i = 0; i < ClusSize; i++) {
    ...
    for (k = firstB; k < lastB; k++) {
      ...
      for (j = 0; j < POS_MAX; j++) {
        ...
        theta[j] += cosf(dumArg);
        zeta[j] +=
            sinf(dumArg); // Since I am taking the average just keep adding
      }
    }
    // printf("Next bead\n");
  }
  ...
  //  GyrTen_{ij} = 1/N sum_1^N (r_i-com_i)(r_j-com_j) so just take the sums and
  //  divide at the end
  for (i = 0; i < ClusSize; i++) {
    firstB = chain_info[naCluster[ClusIndex][i]][CHAIN_START];
    lastB = firstB + chain_info[naCluster[ClusIndex][i]][CHAIN_LENGTH];
    ...
    for (k = firstB; k < lastB; k++) {
      for (j = 0; j < POS_MAX; j++) {
        for (j2 = j; j2 < POS_MAX; j2++) {
          ...
          // 0 = xx; 1 = yy; 2 = zz; 3 = xy; 6 = xz; 4 = yz; Need smarter
          // indexing
        }
      }
    }
...
/// TODO: Update this for the new version
void GyrTensor_GyrRad(
    void) {        // Calculates the gyration tensor for the whole system
  int i, k, j, j2; // Basic indecies for loops
  for (i = 0; i < 7; i++) {
    fGyrTensor[i] = 0.;
  } // Initializing
...
/// GyrTensor_GyrRad_Avg - calculates the total radius of gyration of the
/// system, while not being smart about the periodic boundaries. This is used as
/// a proxy to detect phase separation, but is a relic of the old formalism. The
/// RDF should be used in general. Although this can be used without the need
/// for a non-interacting prior.
void GyrTensor_GyrRad_Avg(void) {
  /*
  Only calculates the diagonals of the gyration tensor, and calculates the sum
  of the diagonals. Remember that Rg^2 = Tr(GyrTen) so we only need to calculate
  the diagonals, and then then sum. I shall borrow most of the code from above,
  and so read GyrTensor_ClusterSpecific for what's happening here.
  */
  ...
  for (i = 0; i < tot_beads; i++) {
    for (j = 0; j < POS_MAX; j++) {
      ...
      dumArg[j] =
          dumArg[j] > nBoxSize[i] / 2 ? nBoxSize[i] - dumArg[j] : dumArg[j];
      fGyrTensor[j] += (float)(dumArg[j] * dumArg[j]);
    }
  }

  fSysGyrRad += sqrtf((fGyrTensor[0] + fGyrTensor[1] + fGyrTensor[2]) /
                      (float)tot_beads); // Adding to the total fSysGyrRad to be
                                         // averaged at the end.
  nTotGyrRadCounter++; // Remembering that we have calculated the radius; this
...
/// The way it is set up, the indexing goes through the diagonal and then 0-1,
/// 0-2, ... 0-N, 1-2, ... 1-N and so on
...
/// RDF_ComponentWise_Avg - calculates the pair-distribution of the system where
/// every bead acts as the center of a radial histogram of pairs. Note that dr =
/// 1/4 lattice units.
void RDF_ComponentWise_Avg(void) {
  /*
  Calculates the RDF and adds it all up so that it can be averaged out at the
  end of the run.
  */
  ...
  for (i = 0; i < tot_beads; i++) {
    ...
    for (j = i + 1; j < tot_beads; j++) {
      ...
      // Note that Dist_BeadToBead(i,j) automatically ensures no distance is
      // greater than (L/2)*sqrt(3)
...
/// Check_LinkerConstraint - if I move beadID to tmpR, do I still satisfy the
/// linker lengths for beadID? \param beadID \param tmpR \return 1 means all is
/// good, 0 means bad.
int Check_LinkerConstraint(const int beadID, const int *tmpR) {
  // Check if the proposed new location for beadID is such that all the linkers
  // are unbroken.
  ...
  while (idx < MAX_BONDS && topo_info[beadID][idx] !=
                                -1) { // Keep going till we run out of partners
    bondPartner = topo_info[beadID][idx];
    if (Dist_PointToPoint(bead_info[bondPartner], tmpR) >
        1.74 * (float)linker_len[beadID][idx]) {
...
/// Check_MTLinkerConstraint - if I move beadID and all of it's covalent bonded
/// beads to the locations stored in tmpR[][], would all the linker constraints
/// be satisfied? \param beadID \param tmpR \return
int Check_MTLinkerConstraint(int beadID, int (*tmpR)[POS_MAX]) {
  ...
  while (curID != -1 && canI == 1) {
    ...
    while (bPart != -1 && idx < MAX_BONDS) {
      if (Dist_BeadToBead(curID, bPart) >
          1.74 * (float)linker_len[curID][idx]) {
        canI = 0;
        break;
      }
      bPart = topo_info[curID][++idx];
...
void Calc_CenterOfMass_OfSystem(lDub *tmpR) {
  ...
  for (j = 0; j < POS_MAX; j++) {
    if ((zeta[j] == 0.) &&
        (xi[j] == 0.)) { // If both 0, then undefined, so skip.
      nCheck[j] = 0;
    } else {
      nCheck[j] = 1;
    }
...
void RadDen_Avg_MolTypeWise_FromMolTypeCen_Old_CorrectVersion(void) {
  ...
  for (cur_type = 0; cur_type < tot_chain_types;
       cur_type++) { // Go through each molType's center
  ...
  )); myBin = (int) (4.*xDis); ldRadDen_Arr[RadDenArr_Index(0, thisComp, myBin)]
  += 1.0;
          }
      }
  }
...
void RadDen_Avg_MolTypeWise_FromMolTypeCen(void) {
  ...
  for (cur_type = 0; cur_type <= tot_chain_types; cur_type++) {
    ...
    for (i = 0; i < tot_beads; i++) {
      ...
      thisType = chain_info[thisType][CHAIN_TYPE];
      thisComp =
          nRadDen_CompShift + RadDen_ComponentIndex(cur_type - 1, thisType);
      xDis = Dist_BeadToPoint(i, COM_int);
      myBin = (int)(4. * xDis);
      ldRadDen_Arr[RadDenArr_Index(0, thisComp, myBin)] += 1.0;
    }
