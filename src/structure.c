...
int Check_System_Structure(void) {
    ...
    for (i = 0; i < tot_beads; i++) {
        ...
        if (i - naTotLattice[Lat_Ind_FromVec(tmpR)] != 0) { // This means there is a mismatch between where the bead is
                                                            // and where the lattice thinks the bead is
        ...
        while (topo_info[i][idx] != -1 && idx < MAX_BONDS) {
            ...
            if (Dist_BeadToBead(i, bondPart) > 1.74 * linker_len[i][idx]) {
                printf("Bad beads! %d\t(%d %d %d)\t\tTopo:(%d %d "
                       "%d)\t\tLinkers:(%.5f\t%.5f\t%.5f)\n",
                       i, bead_info[i][0], bead_info[i][1], bead_info[i][2], topo_info[i][0], topo_info[i][1],
...
void GyrTensor_GyrRad_Avg(void) {
    ...
    */
    ...
    fSysGyrRad +=
        sqrtf((fGyrTensor[0] + fGyrTensor[1] + fGyrTensor[2]) / (float)tot_beads); // Adding to the total fSysGyrRad to
