...
void Memory_Initialization_AtStart(void) {
  int i, j;
  naTotLattice =
      (lInt *)malloc(nBoxSize[0] * nBoxSize[1] * nBoxSize[2] * sizeof(lInt));
  ...
  if (nReport[REPORT_NETWORK] != 0) {
    ...
    for (i = 0; i < nTot_CycleNum; i++) {
      ...
      if (ld_TOTCLUS_ARR[i] == NULL) {
        printf("Malloc Failed for Cluster! Crashing. Probably ran out of "
               "memory. Reduce number of chains.\n");
        exit(1);
      }
    }
    ldMOLCLUS_ARR = malloc((tot_chain_types * tot_chains) * sizeof(lLDub));
    ld_TOTMOLCLUS_ARR =
        malloc((nTot_CycleNum * tot_chain_types * tot_chains) * sizeof(lLDub));
  ...
  if (nReport[REPORT_RDFTOT] != 0) {
    ld_TOTRDF_Arr =
        malloc((nTot_CycleNum * nRDF_TotComps * nRDF_TotBins) * sizeof(lLDub));
  ...
  if (nReport[REPORT_COMDEN] != 0) {
    ...
    ldRadDen_Arr = malloc((nRadDen_TotComps * nRDF_TotBins) *
                          sizeof(lLDub)); // Same as RDF
    ld_TOTRadDen_Arr = malloc(
        (nRadDen_TotComps * nTot_CycleNum * nRDF_TotBins) * sizeof(lLDub));
  }
  if (nTrajMode != 0) {
    nTraj_FramesPerCycle = nMCStepsPerCycle / nReport[REPORT_CONFIG];
    n_TOTTRAJ_ARR = malloc(sizeof(lInt) * nTraj_FramesPerCycle *
                           (lLong)tot_beads * BEADINFO_MAX);
  }
  Memory_VerifyMalloc();
...
void Global_Array_Initialization_AtStart(void) {
  ...
  for (i = 0; i < nBoxSize[0] * nBoxSize[1] * nBoxSize[2];
       i++) {             // Initializing the lattice
  ...
  for (i = 0; i < MAX_AA; i++) {
    for (j = 0; j < MAX_AA; j++) {
      if (fEnergy[i][j][E_SC_SC] !=
          0.0) { // Seeing if this beadType rotationally interacts.
        nBeadTypeIsSticker[i] = 1;
      }
      if (fEnergy[i][j][E_OVLP] !=
          0.0) { // Seeing if this beadType interacts via overlap
        nBeadTypeCanOvlp[i] = 1;
      }
      if (fEnergy[i][j][E_CONT] !=
          0.0) { // Seeing if this beadType interacts via contact
        nBeadTypeCanCont[i] = 1;
      }
      if (fEnergy[i][j][E_F_SOL] !=
          0.0) { // Seeing if this beadType has solvation energy
...
void Initial_Conditions_Simple(void) {
  /*
  Generates initial conditions for a system with topology information. The idea
  is to see if the selected bead has been placed or not. If it has been placed,
  we move on to sequentially placing all of it's bonded beads within linker
  constraints. Therefore, each bead shall sprout forth its partners. Since I
  like goto statements, they have been used. If a bead has not been placed, it's
  first checked if one of its bonding partners might have been placed, and will
  thus act as an anchor. This means that for each chain we need to know which
  beads have already been placed, thus a temporary list is used.
  */
  ...
  int fB, lB; // Used to track the first and last bead of a particular chain for
              // looping.
  ...
  for (i = 0; i < MAX_CHAINLEN;
       i++) { // initialize the list where -1 signifies emptiness.
    temp_list[i] = -1;
  }
  for (j = 0; j < POS_MAX;
       j++) { // Initialize the coordinate arrays to some numbers.
  ...
  for (k = 0; k < tot_chains; k++) {
    ...
    for (i = fB; i < lB; i++) { // Going bead-bead in this chain.
      ...
      for (idy = 0; idy < list_it; idy++) {
        if (i ==
            temp_list[idy]) { // This means this bead has already been placed.
          goto SproutForth;
        }
      }
      ...
      // Let's go through the bonded partners for this bead and see if one can
      // be used as an anchor.
      while (topo_info[i][idx] != -1 &&
             idx <
                 MAX_BONDS) { // Again, -1 signifies that no bonded bead exists.
        bondPart = topo_info[i][idx];
        // Checking if thisBead has been placed already, and can be used as
        // anchor.
        for (idy = 0; idy < list_it; idy++) {
          if (bondPart == temp_list[idy]) { // Using bondPart as an anchor
            goto FoundAnchor; // This is just so we don't keep going over the
                              // list needlessly and just pick the first one.
          }
        }
        idx++;
        // If we got here, this means no bonded partner has already been placed,
        // so no anchor.
        bondPart = -1;
      }
    ...
    FoundAnchor:
      if (bondPart != -1) { // We found an anchor.
        ...
        if (TlCnt == 5000000) {
          printf("Not enough space in the lattice. Crashing. Maybe try "
                 "increasing max trials, "
      ...
      } else { // There was no appropriate anchor. So we can put this bead
               // wherever.
        for (j = 0; j < POS_MAX; j++) {
          tmpR[j] = rand() % nBoxSize[j];
        }
        ...
        TlCnt = 0;
        while (
            naTotLattice[Lat_Ind_FromVec(tmpR)] != -1 &&
            TlCnt <
                5000000) { // Keep looping till we find an empty lattice site.
          TlCnt++;
          for (j = 0; j < POS_MAX;
               j++) { // Generate a random point in the lattice.
            tmpR[j] = rand() % nBoxSize[j];
          }
        }
        if (TlCnt == 5000000) {
          printf("\n\nNot enough space in the lattice for first bead, bruh. "
    ...
    SproutForth:
      ...
      while (topo_info[i][idx] != -1 &&
             idx <
                 MAX_BONDS) { // Again, -1 signifies that no bonded bead exists.
        bondPart = topo_info[i][idx];
        // If the bead has already been dealt with, move on to next potential
        // bondPartner
        for (idy = 0; idy < list_it; idy++) {
        ...
        if (TlCnt == 5000000) {
          printf("\n\nNot enough space in the lattice. Crashing. Maybe try "
                 "increasing max trials, "
...
void Initial_Conditions_FromFile(void) {
  ...
  tmpBeads = 0;
  while (fgets(strLine, sizeof(strLine), infile) != NULL) {
    sscanf(strLine, "%d %d %d %d %d %d %d", &i, &tmp_beadinfo[BEAD_TYPE],
           &tmp_beadinfo[BEAD_CHAINID], &tmp_beadinfo[POS_X],
           &tmp_beadinfo[POS_Y], &tmp_beadinfo[POS_Z],
  ...
  if (tmpBeads != tot_beads) {
    printf("Given restart file does not have the same number of beads as the "
           "provided structure.");
    printf("Crashing!\n");
    exit(1);
  }
...
void Initial_Conditions_BreakBonds(void) {
  ...
  printf(
      "Since system has thermalization cycle, deleting all physical bonds!\n");
}
...
void Calculate_Rot_Bias(float CurrentTemp) {
  ...
  for (i = 0; i < MAX_AA; i++) {
    for (j = 0; j < MAX_AA; j++) {
      dbias_bolt_fac[i][j] =
          (lLDub)expl(-(lLDub)fEnergy[i][j][E_SC_SC] / (lLDub)CurrentTemp);
