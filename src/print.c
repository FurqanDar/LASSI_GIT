...
void Print_Matrix(char *strTitle, int nSeqEn,
                  float fArray[MAX_AA][MAX_AA][MAX_E], int param);
...
void Write_MCMove(char *filename, long nGen, float fMCTemp) {
  ...
  if (nGen == -1) {
    fprintf(fp,
            "#Steps ,and Temp, are followed by (rej, acc) for each MC Move.\n");
    fprintf(
        fp,
        "#(nStep, T) TRANS CLSTER SM_CLSTER FACE LOCAL DBPVT PIVOT BR_ROT\n");
    for (i = 0; i < MAX_MV; i++) {
      MCAccepMat[0][i] = 0;
      MCAccepMat[1][i] = 0;
    ...
    for (i = 1; i < MAX_MV; i++) {
      ...
      MCAccepMat[0][i] = 0;
      MCAccepMat[1][i] = 0;
      // This way the print function will zero out the matrix every time we
      // print to a file!
    }
    fprintf(fp, "\n");
  }
...
void Print_LogToScreen(long nGen, int run_it) {
  ...
  printf("Total E  = %.3e;\tIso E = %.3e;\tAniso E = %.3e;\n", faCurrEn[E_TOT],
         faCurrEn[E_OVLP], faCurrEn[E_SC_SC]);
  printf("Percolation Parameter Is: %.3f\n",
         ((float)nLargestClusterRightNow) / ((float)nTotClusCounter + 0.0001) /
             (float)tot_chains);
  int i, j;
  printf("Acceptance Ratios:\n");
  for (i = 1; i < MAX_MV; i++) {
    printf("%.2f\t",
           100. * (float)MCAccepMat[1][i] /
               ((float)MCAccepMat[0][i] + 0.00001 + (float)MCAccepMat[1][i]));
  }
  printf("\n\n");
}
  ...
  if (nTrajMode == 1) {
    ...
    if (run_it >= 0) {
      Write_Trajectory(fileStruct,
                       nGen + nMCPreSteps + run_it * nMCStepsPerCycle);
    } else {
...
void Write_Trajectory(char *filename, long nGen) {
  // Writes the trajectory in LAMMPS format. To be viewed with VMD (Hopefully).
  // Read the LAMMPS dump documentation for the actual formate of the file
  FILE *fp;
  if (nGen == -1) {
    ...
    for (i = 0; i < tot_beads; i++) {
      fprintf(fp, "%d %d %d %d %d %d %d\n", i, bead_info[i][BEAD_TYPE],
              bead_info[i][BEAD_CHAINID], bead_info[i][POS_X],
              bead_info[i][POS_Y], bead_info[i][POS_Z],
...
void Write_Saved_Trajectory(char *filename, const int run_it) {
  ...
  for (i = 0; i < nTrajCurFrame; i++) {
    ...
    if (run_it >= 0) {
      fprintf(fp, "%ld\n",
              nMCPreSteps + run_it * nMCStepsPerCycle +
...
void Print_Key(
    void) { // should be output-dependent (stdout, stderr, other files)
  ...
  printf("Monomer density         = %1.1e\n",
         (float)tot_beads / (float)nBoxSize[0] / (float)nBoxSize[1] /
             (float)nBoxSize[2]);
  printf("\n");
  ...
  printf("MC Temperatures: (First, Last) = (%.2f, %.2f)\n", fKT,
         fKT + (float)(nTot_CycleNum - 1) * fdelta_temp);
  ...
  */
  ...
  for (i = 0; i < tot_beads; i++) {
    for (j = 0; j < MAX_BONDS; j++) {
      if (topo_info[i][j] !=
          -1) { // There is a bond between i and topo_info[i][j]
        numBonds++;
      }
    }
  }
  ...
  for (i = 0; i < tot_beads; i++) {
    fprintf(fp, "%d %d %d 0.0 %d %d %d\n", i, bead_info[i][BEAD_CHAINID],
            bead_info[i][BEAD_TYPE], bead_info[i][POS_X], bead_info[i][POS_Y],
            bead_info[i][POS_Z]);
  ...
  for (i = 0; i < tot_beads; i++) {
    for (j = 0; j < MAX_BONDS; j++) {
      if (topo_info[i][j] !=
          -1) { // There is a bond between i and topo_info[i][j]
        fprintf(fp, "%d %d %d %d\n", k, linker_len[i][j] / (int)fLinkerLength,
                i, topo_info[i][j]);
        k++;
      }
    }
...
void Write_SysProp(char *filename) {
  ...
  for (j = 0; j < nRDF_TotComps; j++) {
    for (i = 0; i < nRDF_TotBins; i++) {
      ...
      fprintf(fp, "%LE\t",
              ldRDF_Arr[RDFArr_Index(0, j, i)] / (float)nRDFCounter);
    }
    fprintf(fp, "\n");
  }
  fprintf(fp, "\n#Done");
...
void Write_TotalSysProp(char *filename, int run_it) {
  /* This function writes one large file with all the averaged values from each
   * run_cycle. run_it let's the function know how many cycles to write. As
   * opposed to Write_SysProp, each of the relevant averaged parameters will be
   * in their appropriately named files. The naming convention shall be:
   * filename_*.dat where * will be GR, CLUS and RDF for the different measured
   * quantities. Within each file, each run_cycle shall be written one-by-one in
  ...
  if (nReport[REPORT_COMDEN] != 0) {
    ...
    fprintf(
        fp,
        "#Density distribution from the COM outwards. Order is ChainType.\n");
    for (i = 0; i < run_it; i++) {
      fprintf(fp, "#Run_Cycle = %d\n", i);
  ...
  if (nReport[REPORT_NETWORK] != 0) {
    ...
    fprintf(
        fp,
        "#Cluster Histograms: 1st column is largest cluster, and then clusters "
...
void Print_Data(long nGen, int run_it) {
  ...
  if (run_it == -1) {
    ...
    if (nGen == -1) {
      if (nReport[REPORT_CONFIG] != 0) {
        ...
        Write_TopFile(
            fileStruct); // Write the topology file. Only need to write once
        if (nTrajMode != 1) {
          sprintf(fileStruct, "%s_trj.lammpstrj",
                  strReportPrefix); // Naming convention for trajectory files.
          Write_Trajectory(fileStruct, -1); // This opens a new trajectory file;
      ...
      if (nReport[REPORT_ENERGY] != 0) {
        ...
        Write_Energy(
            fileEnergy,
            -1); // Open a new energy file; each run_it will have its own
      }
      if (nReport[REPORT_MCMOVE] != 0) {
        sprintf(fileMCMove, "%s_mcmove.dat", strReportPrefix);
        Write_MCMove(
            fileMCMove, -1,
            0.0); // Open a new MCInfo file; each run_it will have its own
      }
      if (nReport[REPORT_NETWORK] != 0 || nReport[REPORT_RDFTOT] != 0) {
        sprintf(fileSysProp, "%s_SysProp.dat",
                strReportPrefix); // Naming convention or SysProp files; each
  ...
  if (run_it == 0 && nGen == -1) { // Write out equilibrium trajectory
    if (nReport[REPORT_CONFIG] != 0) {
      if (nTrajMode == 1) {
  ...
  if (run_it == 0 && nGen > 0) {
    ...
    if (nReport[REPORT_RDFTOT] !=
        0) { // SysProp is printed outside of this function in main.c, lol
  ...
  if (run_it > 0) {
    if (nGen == -1) {
      if (nReport[REPORT_ENERGY] != 0) {
        ...
        Write_Energy(
            fileEnergy,
            0); // Last frame from previous run_it is first frame of this one
      }
      if (nReport[REPORT_MCMOVE] != 0) {
      ...
      if (nReport[REPORT_NETWORK] != 0 || nReport[REPORT_RDFTOT] != 0) {
        sprintf(fileSysProp, "%s_%d_SysProp.dat", strReportPrefix,
                run_it); // Naming convention or SysProp files; each run_it
                         // should have its own
      }
    } else {
      if (nReport[REPORT_LOG] != 0) {
      ...
      if (nReport[REPORT_CONFIG] != 0) {
        if (nGen % nReport[REPORT_CONFIG] == 0) {
          ...
          HandleTrajectory(fileStruct, run_it, nGen);
          // Write_Trajectory(fileStruct, nGen + (run_it * nMCStepsPerCycle));
        }
      }
      if (nReport[REPORT_ENERGY] != 0) {
        if (nGen % nReport[REPORT_ENERGY] == 0) {
    ...
    for (i = 0; i < nRadDen_TotComps; i++) {
      for (j = 0; j < nRDF_TotBins; j++) {
        ld_TOTRadDen_Arr[RadDenArr_Index(run_it, i, j)] =
            ldRadDen_Arr[RadDenArr_Index(0, i, j)] /
            (long double)nRadDenCounter;
      }
    }
  }
  if (nReport[REPORT_NETWORK] != 0) {
    ld_TOTCLUS_ARR[run_it][0] =
        (long double)nLargestClusterRightNow / (long double)nTotClusCounter;
    for (i = 1; i <= tot_chains; i++) {
      ld_TOTCLUS_ARR[run_it][i] =
          (long double)naClusHistList[i] / (long double)nTotClusCounter;
    }
    for (i = 0; i < tot_chains; i++) {
      for (j = 0; j < tot_chain_types; j++) {
        ld_TOTMOLCLUS_ARR[MolClusArr_Index(run_it, j, i)] =
            ldMOLCLUS_ARR[MolClusArr_Index(0, j, i)] /
            (long double)nTotClusCounter;
      }
    }
    ld_TOTGYRRAD_ARR[run_it][0] =
        (long double)fSysGyrRad / (long double)nTotGyrRadCounter;
