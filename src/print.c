...
long
TrajArr_Index (const int beadID, const int nFrameNumber, const int beadProp) {
...
void
Write_ClusterDist (char* filename, long nGen) {
    FILE* fp;
    int i;
    if ( nGen == -1 ) {
...
void
Write_GyrTen (char* filename, long nGen) {
    FILE* fp;
    int i;
    if ( nGen == -1 ) {
        fp = fopen (filename, "w"); // overwrite
...
void
Write_MCMove (char* filename, long nGen, float fMCTemp) {
    FILE* fp;
    int i; // Iterator
    if ( nGen == -1 ) {
        fp = fopen (filename, "w"); // overwrite
...
void
Print_LogToScreen (long nGen, int run_it) {
    printf ("Step       %.2e\n", (float) nGen);
    printf ("Run Cycle: %d\n", run_it);
...
/// \param nGen
void
Write_Energy (char* filename, long nGen) {
    FILE* fp;
    if ( nGen == -1 ) {
        fp = fopen (filename, "w"); // overwrite
    } else {
...
void
HandleTrajectory (char* fileStruct, const int run_it, const long nGen) {
    if ( nTrajMode == 1 ) {
...
/// \param nGen
void
Write_Trajectory (char* filename, long nGen) {
...
/// \param curFrame
void
Save_Trajectory (const long nGen, const long curFrame) {
...
/// \param nGen
void
Write_Saved_Trajectory (char* filename, const int run_it) {
    // Writes the trajectory in LAMMPS format. To be viewed with VMD
    // (Hopefully). Read the LAMMPS dump documentation for the actual formate of
    // the file
    FILE* fp;
...
void
Print_Key (void) { // should be output-dependent (stdout, stderr, other files)
...
/// \param param
void
Print_Matrix (char* strTitle, int nSeqEn, float fArray[MAX_AA][MAX_AA][MAX_E], int param) {
    int nLen;
    nLen = nSeqEn;
    int i, j;
    for ( i = 0; i < nLen + 1; i++ ) {
...
void
Write_RDF_ComponentWise (char* filename, long nGen) {
    FILE* fp;
    if ( nGen == -1 ) {
        fp = fopen (filename, "w"); // overwrite
    } else {
...
void
Write_SysProp (char* filename) {
    FILE* fp;
    int i, j;
    fp = fopen (filename, "w");
    fprintf (fp, "#Contains various averaged quatities.\n");
    // Gyration Radius
    fprintf (fp, "#Total Rg\n%f\t%f\n#Cluster Hist\n", fSysGyrRad / (float) nTotGyrRadCounter,
             (float) nBoxSize[0] / 2.);
    // Cluster Histogram
...
void
Write_TotalSysProp (char* filename, int run_it) {
...
void
Print_Data (long nGen, int run_it) {
    ...
    if ( run_it > 0 ) {
        if ( nGen == -1 ) {
            ...
            if ( nReport[REPORT_RDFTOT] != 0 ) { // SysProp is printed outside of this function in main.c,
                                                 // lol
                if ( nGen % nReport[REPORT_RDFTOT] == 0 && nGen > nMCStepsPerCycle / 2 ) {
                    RDF_ComponentWise_Avg();
                }
            }
            if ( nReport[REPORT_COMDEN] != 0 ) { // SysProp is printed outside of this function in main.c,
                                                 // lol
                if ( nGen % nReport[REPORT_COMDEN] == 0 && nGen > nMCStepsPerCycle / 2 ) {
                    RadDen_Avg_MolTypeWise_FromMolTypeCen();
                }
            }
            if ( nReport[REPORT_NETWORK] != 0 ) { // SysProp is printed outside of this function in main.c,
                                                  // lol
                if ( nGen % nReport[REPORT_NETWORK] == 0 && nGen > nMCStepsPerCycle / 2 ) {
                    Clus_Perform_Analysis();
                    GyrTensor_GyrRad_Avg();
