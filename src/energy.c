    ...
    case 4:
      ...
      if (totEn <= 100.) {
        totEn = (fCuTemp - fKT) * (totEn + 0.2);
          ...
          totEn = ;(fCuTemp - fKT*0.) * ((float) tot_beads + 1. / (totEn +
      0.02));
      }*/
      else {
        totEn = 0.;
...
/// \param beadID
/// \return
float Energy_Anisotropic(
    const int beadID) { // Calculates the SC-SC energy of the bead in question.
  float totEn = 0.0;    // Storing total overlap energy
  if (bead_info[beadID][BEAD_FACE] != -1) {
    totEn +=
        fEnergy[bead_info[beadID][BEAD_TYPE]]
               [bead_info[bead_info[beadID][BEAD_FACE]][BEAD_TYPE]][E_SC_SC];
  }
  return totEn;
}
...
float Energy_Anisotropic_Self(
  ...
  if (bP != -1) { // This bead does have a bond
    if (bead_info[beadID][BEAD_CHAINID] == bead_info[bP][BEAD_CHAINID]) {
      totEn += fEnergy[bead_info[beadID][BEAD_TYPE]][bead_info[bP][BEAD_TYPE]]
                      [E_SC_SC];
    }
  }
  return totEn;
...
/// \param beadID
/// \return
float Energy_Anisotropic_For_Chain(
    const int beadID) { // Calculates the SC-SC energy of the bead in question.
  float totEn = 0.0;    // Storing total overlap energy
  int bP = bead_info[beadID][BEAD_FACE];
  if (bP != -1) { // This bead does have a bond
...
float Energy_Anisotropic_Contiguous_Range(const int beadID,
  ...
  int bP = bead_info[beadID][BEAD_FACE];
  if (bP != -1) { // This bead does have a bond
    if (bead_info[beadID][BEAD_CHAINID] ==
        bead_info[bP][BEAD_CHAINID]) {                 // Intra-molecular
...
float Energy_Anisotropic_With_List(const int beadID, const int *bead_list,
  ...
  if (bP != -1) { // This bead does have a bond
    for (i = 0; i < list_size;
         i++) { // Checking if we should worry about double counting
      if (bP == bead_list[i]) {
        check_bead = 1;
        break;
      }
...
/// where \f$\epislon$\f represents the overlap cost for total overlap, which is
/// forbidden explicitly in LASSI, so we have \f$\epislon/2$\f,\f$\epislon/4$\f
/// and \f$\epislon/8$\f with increasing distance. \param beadID \return
float Energy_Isotropic_Old(
    int beadID) {    // Calculate Contact and Overlap energy of bead beadID
  float totEn = 0.0; // Storing total overlap energy
  int i, j;          // Indecies
  ...
  int BoxRad = nBeadTypeCanCont[resi] == 0
                   ? 1
                   : 3; // No need to search if no cont interactions
    ...
    for (y = -BoxRad; y <= BoxRad; y++) {
      for (z = -BoxRad; z <= BoxRad; z++) {
        ...
        if (secBi != -1 && secBi != beadID) {
          ...
          if (abs(x) <= 1 && abs(y) <= 1 &&
              abs(z) <= 1) { // Want solvation radius to be 1
...
/// Energy_CONT serves as the base function for energy calculations depending on
/// the distance. \param fEn: The interaction energy. Should be
/// fEnergy[i][j][E_CONT] \param xDis: The distance on the lattice. \return
/// totEn = fEn / (xDis)
...
/// forbidden explicitly in LASSI, so we have \f$\epislon/2$\f,\f$\epislon/4$\f
/// and \f$\epislon/8$\f with increasing distance. \param beadID \return
float Energy_Isotropic(
    const int beadID) { // Calculate Contact and Overlap energy of bead beadID
  float totEn = 0.0;    // Storing total overlap energy
  int i, j;             // Indecies
  int tmpR[POS_MAX], tmpR2[POS_MAX];
  int x, y, z;     // Lattice indecies
  ...
  neighborNum = NeighborList_StoreNeighborsAndDistance(
      beadID, tmpR, BoxRad, neighborList, neighborDist);
...
/// account for double counting \param beadID \return
float Energy_Isotropic_Self(
    const int beadID) { // Calculate Contact and Overlap energy of beadID but
...
/// \param beadID
/// \return
float Energy_Isotropic_For_Chain(
    const int beadID) { // Calculate Contact and Overlap energy of beadID
...
float Energy_Isotropic_Contiguous_Range(
    const int beadID, const int smallest_bead,
  ...
  // This assumes that every bead between smallest_bead and largest_bead will be
  // looped over
  float totEn = 0.0; // Storing total overlap energy
  int i, j;          // Indecies
  int tmpR[POS_MAX], tmpR2[POS_MAX];
...
float Energy_Isotropic_With_List(
    const int beadID, const int *bead_list,
...
// TODO: Change total energy calculation so that it properly decomposes the
// system energy into individual components.
/// Energy_Total_System - calculates the total energy of the system using the
/// functions above. Note the factor of 1/2 to account for double counting since
/// all the energy contributions (for now) are pair-wise. Furthermore,
/// faCurrEn[] stores the two energies separately as well.
...
/// chain.
float Energy_Of_Chain(
    const int chainID) { // Calculates the energy of the given chain.
  float totEn = 0.0;
  int i; // Looping index
...
float Energy_Of_Chain_Self(
    const int chainID) { // Calculates only intra-molecular interactions
  float totEn = 0.0;
  int i; // Looping index
  int fB = chain_info[chainID][CHAIN_START];
  int lB = chain_info[chainID][CHAIN_START] + chain_info[chainID][CHAIN_LENGTH];
...
/// still correct.
float Energy_Of_Chain_OLD(
    int chainID) { // Calculates the energy of the given chain
  float totEn = 0.0;
  int i; // Looping index
