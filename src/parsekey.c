...
/// Have to painstakingly go through every different keyword that exists in the keyfile. Everytime you want to add
/// something to the keyfile, do it here and also the python scripts so that everything is consistent. \param filename
/// \return
...
/// str2farr - converts the string array to an array of floats. The nice thing about C is the ease of string
/// manipulation. used to generate matrices from the energy file. \param strRaw \param fArray \return Changes fArray to
...
void Parse_StructureFile(char *filename) {
    ...
    Each molecule is within NEW{\0 ... \0}END which act as keywords. After NEW{, the next line contains the number of
    copies to be made of this molecule. It is also expected that whenever a bead is listed, ALL of its bonds follow in
    the next lines. E.G A molecule with 5 branches where each branch has 2 beads would be:
    ###
    NEW{
    nCopies
    0 0 1 1
    ...
    bead (topo_info[i][j]). After each molecule is read, nCopies copies are made, and THEN, the next line in the file is
    read.
    */
    FILE *inFile;
        ...
        if (nFlag == 1) { // This signifies that a new molecule has started
            ...
            while (fgets(strLine, sizeof(strLine), inFile) != NULL && nFlag == 1) {
                ...
                if (bead_info[curID][BEAD_TYPE] ==
                    ...
                    nCursor = 0; // This is a counter for number of bonds, which should reset when you have a 'new'
                                 // bead.
                }
