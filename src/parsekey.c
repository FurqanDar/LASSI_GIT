...
int Parse_Keyfile(char *filename) {
  ...
  while (fgets(strLine, sizeof(strLine), infile) != NULL) {
    ...
    if (strKeyword[0] == '#') { // if the first character is #,
      ...
      if (strcmp(strKeyword, "BOX_SIZE") == 0) {
        sscanf(strLine, "%*s %s %s %s", strTempArr[0], strTempArr[1],
               strTempArr[2]);
  ...
  if (strStructFile[0] != '\0') {
    ...
    if (nStructFiletype == 0) {
      ...
      printf(
          "Reading restart file provided to generate initial configuration.\n");
      bReadConf = 1;
    } else {
      fprintf(stderr,
              "ERROR: undefined value in STRUCT_FILETYPE of %s.\nCrashing!\n",
              filename);
      exit(1);
    }
  } else {
    ...
    if (strLine[0] == '#') { // All key-words start with '#'
      ...
      if (nFlag == -1) {                     // sticker
        ...
        if (nBeadTypes > MAX_AA) {
          fprintf(stderr,
                  "ERROR: the number of AA types exceeds MAX_AA in %s.\n",
                  strEnFile);
          nRes = 3;
          break;
        }
        ...
        if (nEntry != nBeadTypes) { // If columns not the same as sticker number
          if (nEntry ==
              1) { // If only 1-value, all-values in  that matrix are this one
            ...
            fprintf(stderr,
                    "ERROR: irregular expression in energy matrices of %s.\n",
                    strEnFile);
            nRes = 4;
            break;
          }
  ...
  */
  ...
  while (fgets(strLine, sizeof(strLine), inFile) != NULL && nFlag == -1) {
    ...
    if (strKeyword[0] !=
      ...
      for (i = 0; strLine[i] != '\0';
           i++) { // Keep reading till end of that line
        if (strLine[i] ==
            '#') { // This line now has a comment, so stop reading further
          strLine[i] = '\0'; // Just assume we have reached the end of the line
          break;
        }
      }
      if (strcmp(strKeyword, "NEW{") ==
          0) { // This signifies a new molecule type has been started
        nFlag = 1;
      }
      if (strcmp(strKeyword, "}END") ==
          0) { // This signifies a new molecule type has been started
        nFlag = -1;
      }
    }
    if (nFlag == 1) { // This signifies that a new molecule has started
      ...
      // molecule.
      nChainType++;
      nChainTypeIsLinear[nChainType] =
          1; // Assume all chains are linear to begin with.
      nChainStart += nBEADS;
      nChainID++;
      tot_chains++;
      nBEADS = 0;
      ...
      while (fgets(strLine, sizeof(strLine), inFile) != NULL && nFlag == 1) {
        ...
        sscanf(strLine, "%d %d %d %d", &curID, &curType, &curLinker,
               &curPartner);
        curID += nChainStart; // Accounting for previously defined beads
        if (bead_info[curID][BEAD_TYPE] ==
            -1) { // This is to make sure that each bead is counted once only
        ...
        if (curPartner != -1) { // This bead has a bonded partner
          if (nCursor >
              1) { // This signifies that the chain is not linear because a bead
                   // has more than two bonds because indecies start at 0
            nChainTypeIsLinear[nChainType] = 0;
          }
          curPartner +=
              nChainStart; // Accounts for all beads before, like above.
      ...
      for (k = 1; k < nCopies;
           k++) {              // Now we just copy this molecule nMOL-1 times
        ...
        for (i = 0; i < nBEADS; i++) {
          ...
          for (j = 0; j < MAX_BONDS; j++) {
            ...
            if (topo_info[nTemp][j] != -1) {
              topo_info[curID][j] =
                  topo_info[nTemp][j] +
                  nBEADS; // Because we must account for chain lengths
            }
          }
        }
      }
