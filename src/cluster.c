...
int Clus_Network_ChainCluster_General(int const chainID) {
  ...
  // and to add it to naList

  int i, j; // Loop iterators
  for (i = 0; i <= tot_chains; i++) {
    naList[i] = -1; // Initialize the list where -1 means empty.
    naChainCheckList[i] =
        -1; // Initialize the list where no chain has been visited.
  }
  ...
  curID = chainID;
  naList[clusSize++] =
      curID; // The cluster contains chainID by definition, and ClusSize = 1
  ...
  while (curID != -1) { // Keep going through naList till it is exhausted.
    ...
    lB = fB +
         chain_info[curID][CHAIN_LENGTH]; // Last bead+1 of this chain. Makes
                                          // for-loops easier this way.
    for (i = fB; i < lB; i++) { // Loop over all the beads in this chain and see
                                // if there is a physical bond.
      if (bead_info[i][BEAD_FACE] !=
          -1) { // This means we have a bonding partner.
...
int Clus_Network_ChainCluster_ForTotal(int const chainID) {
  // Updates naList to have all proteins bound to  chainID and it's cluster
  // The idea is to check every bead and see if there is a unique bonded chain,
  // and to add it to naList This one is specifically made to be used in total
  // system network analyses, so naList and naChainCheckList aren't
  ...
  while (curID != -1) { // Keep going through naList till it is exhausted.
    ...
    // Done with this chain, so let's move to the next chain in the cluster, if
    // it exists.
    list_it++; // Going one forward in naList to completely exhaust the tree.
    curID = naList[list_it];
  }
...
void Clus_Network_TotalAnalysis(void) {
  ...
  while (curID < tot_chains && IsUnique == 1) {
    Cluster_length = Clus_Network_ChainCluster_ForTotal(
        curID); // This is the length of curID cluster
    if (Cluster_length > currentLargest) {
      currentLargest = Cluster_length;
      j = ClusNum;
    }

    for (i = 0; i < Cluster_length;
         i++) { // Recording the chains in this cluster
      naCluster[ClusNum][i + 1] = naList[i];
      naList[i] = -1;
    }
    naCluster[ClusNum++][0] = Cluster_length;
    IsUnique = 0; // Assume not unique -- just got analyzed.
    while (curID < tot_chains &&
           IsUnique ==
               0) { // Finding the next chainID that hasn't been analyzed.
      curID++;
...
int Clus_Network_SecondLargestCluster(void) {
  ...
  */
  ...
  if (currentLargest == 1) { // Just single chains
    ...
    curID = 0;
    for (i = 0; i < ClusNum; i++) {
      if (naCluster[i][0] < currentLargest &&
          naCluster[i][0] >= naCluster[curID][0]) {
        curID = i;
      }
    }
  ...
  for (i = 0; i < naCluster[curID][0];
       i++) { // Reupdate naList to have IDs for the second largest cluster
...
int Clus_Network_LimitedCluster(int const chainID) {
  // Updates naList to have all proteins bound to  chainID and it's cluster
  // The idea is to check every bead and see if there is a unique bonded chain,
  // and to add it to naList If Cluster becomes larger than 5, exit and return
  // -1
  int ClusterLimit = 15;
  int i, j; // Loop iterators
  ...
  int fB, lB; // Indecies to track the first and last bead of chains.
  int chainPart;
  int IsUnique = 1; // Tracks if a chain is unique or not. 0 is non-unique, and
                    // 1 is unique.
...
int Clus_Proximity_ChainCluster_ForTotal_IntOnly(int const chainID) {
  ...
  // (+-1,+-1) cube, and to add it to naList This one is specifically made to be
  // used in total system proximity cluster analyses, so naList and
  ...
  while (curID != -1) { // Keep going through naList till it is exhausted.
    ...
    lB = fB +
         chain_info[curID][CHAIN_LENGTH]; // Last bead+1 of this chain. Makes
                                          // for-loops easier this way.
    for (i = fB; i < lB; i++) { // Loop over all the beads in this chain and see
                                // if there is another bead around.
      ...
      for (k = 0; k < MAX_ROTSTATES - 1; k++) {
        ...
        for (j = 0; j < POS_MAX; j++) {
          tmpR2[j] =
              (tmpR[j] + LocalArr[tmpBead][j] + nBoxSize[j]) % nBoxSize[j];
        }
        tmpBead = Lat_Ind_FromVec(tmpR2);
        tmpBead = naTotLattice[tmpBead];
        if (tmpBead != -1) {
          resj = bead_info[tmpBead][BEAD_TYPE];
          if (fEnergy[resi][resj][E_OVLP] >=
              0.) { // If not interacting, or repelling, it is not a bond
            continue;
          }
...
void Clus_Proximity_TotalAnalysis(void) {
  ...
  j = 0;
  while (curID < tot_chains && IsUnique == 1) {
    Cluster_length = Clus_Proximity_ChainCluster_ForTotal_IntOnly(
        curID); // This is the length of curID cluster
    if (Cluster_length > currentLargest) {
      currentLargest = Cluster_length;
      j = ClusNum;
...
int Clus_Proximity_LimitedCluster_IntOnly(int const chainID) {
  // Updates naList to have all proteins bound to  chainID and it's cluster
  // The idea is to check every bead and see if there is a unique bonded chain,
  // and to add it to naList If Cluster becomes larger than 15, exit and return
  // -1
  int ClusterLimit = 15;
  int i, j, k; // Loop iterators
  for (i = 0; i < ClusterLimit; i++) {
...
int Clus_Proximity_LimitedCluster_IntOnly_Check(int const chainID,
  ...
  // The idea is to check every bead and see if there is a unique bonded chain,
  // and to add it to naList If Cluster becomes larger than 15, exit and return
  // -1 Furthermore, it checks while it goes to see if naList[i] == OldList[i].
  // If not, return -1.
  int ClusterLimit = 15;
  int i, j, k; // Loop iterators
  for (i = 0; i < ClusterLimit; i++) {
...
void Clus_Proximity_Distribution_All_MolWise_Avg(void) {
  ...
  */
  ...
  currentLargest = 0;
  while (curID < tot_chains && IsUnique == 1) {
    Cluster_length = Clus_Proximity_ChainCluster_ForTotal_All(
        curID); // This is the length of curID cluster
    // printf("Clus Len: %d\n", Cluster_length);
    naClusHistList[Cluster_length]++; // Adding to that cluster-size bin
    if (Cluster_length > currentLargest) {
...
void Clus_Perform_MolWise_LargestClusters(void) {
  ...
  while (curID < tot_chains && IsUnique == 1) {
    Cluster_length = Clus_Perform_ChainCluster_ForTotal(
        curID); // This is the length of curID cluster
...
void Clus_Find_LargestClusters(void) {
  // Just a function that picks the right analysis routine given the clustering
  // mode
