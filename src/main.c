...
int main(int argc, char *argv[]) {
  ...
  if (bReadConf == 0) {
    ...
    if (nMCPreSteps > 0) { // Remember that all thermalizing variants of MC
                           // moves cannot handle bonds.
  ...
  for (nGen = 0; nGen < nMCPreSteps;
       nGen++) { // Intentionally not performing any data acquisition in the
                 // thermalizing phase.
